...
export function EventRow({ event }: { event: LiveEvent }) {
  ...
  return (
    <div
      className="group grid grid-cols-1 items-end gap-6 border-t border-[#1a1a1a] py-8 transition-colors duration-300 hover:border-magenta sm:grid-cols-12 sm:gap-4 lg:py-10"
    >
      <div className="flex items-center gap-6 sm:col-span-3">
        <p className="font-display text-5xl font-normal leading-none text-magenta tabular-nums sm:text-6xl">
          {formatEventDay(event.date)}
        </p>
        <p className="font-display text-2xl font-normal uppercase leading-none text-[#F2F0EA]">
          {formatEventMonth(event.date)}
        </p>
      </div>

      <div className="sm:col-span-5">
        <p className="font-display text-2xl font-normal uppercase leading-tight text-[#F2F0EA] sm:text-3xl">
          {event.city}
        </p>
      ...
      <div className="flex items-center gap-4 sm:col-span-4 sm:justify-end">
        {event.ticketStatus === "soldout" ? (
          ...
          <a
            ...
            className="inline-flex items-center gap-3 bg-[#F2F0EA] px-7 py-3 font-mono text-[0.65rem] uppercase tracking-[0.25em] text-[#080808] transition-colors duration-300 hover:bg-magenta hover:text-[#F2F0EA]"
          >
            Get Tickets
