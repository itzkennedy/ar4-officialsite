    ...
    <div className={`flex flex-wrap items-center gap-x-6 gap-y-3 ${className}`}>
      {links.map((link) => {
        ...
        return (
          ...
          >
            <Icon className="h-4 w-4 text-[#9b9b93] transition-colors group-hover:text-[#a8342a]" />
            {link.platform}
          </a>
        );
      })}
...
export function ListenButton({
  ...
  const variants = {
    primary:
      "bg-[#F2F0EB] text-[#080808] hover:bg-[#a8342a] hover:text-[#F2F0EB]",
    outline:
      "border border-[#2a2a2a] text-[#F2F0EB] hover:border-[#a8342a] hover:text-[#a8342a]",
    ghost: "text-[#F2F0EB] hover:text-[#a8342a]",
  };
